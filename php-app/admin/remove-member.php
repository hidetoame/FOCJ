...
// POSTデータがある場合は退会処理を実行
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['remove-member-confirm'])) {
    // 退会処理を実行
    ...
    $stmt->execute([
        ':id' => $id,
        ':withdrawn_by' => $_SESSION['admin_username'] ?? 'admin',
        ':reason' => '管理者による退会処理'
    ]);
    
    // 退会完了画面へリダイレクト
    header('Location: remove-member-complete.php?id=' . $id);
    exit;
}
...
// formタグを追加（テンプレートにはformがないため）
$html = preg_replace(
    '/<div class="member-detail-contents">/',
    '<form action="remove-member.php?id=' . $id . '" method="post"><div class="member-detail-contents">',
    $html, 1
);

// 退会処理ボタンをsubmitボタンに変更（初期状態は無効）
$html = str_replace(
    '<a href="C7_remove-member-complete.html" class="button button--primary button--disable" id="remove-member-button">退会処理をする</a>',
    '<button type="submit" name="remove-member-confirm" value="1" class="button button--primary button--disable" id="remove-member-button" disabled>退会処理をする</button>',
    $html
);

// formの閉じタグを追加
$html = preg_replace(
    '/(<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/body>)/',
    '</form>$1',
    $html, 1
);
...
$html = preg_replace('/>03-0000-0000<\/div>/', '>' . h($member['phone_number'] ?: '-') . '</div>', $html, 1);
...
// チェックボックス制御のJavaScriptを追加
$javascript = '
<script>
document.addEventListener("DOMContentLoaded", function() {
    const checkbox = document.getElementById("remove-member-confirm");
